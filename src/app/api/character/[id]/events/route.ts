...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const eventsUrl = `${MARVEL_BASE_URL}/characters/${params.id}/events?ts=${auth.ts}&apikey=${auth.apikey}&hash=${auth.hash}&limit=20&orderBy=-startDate`
    console.log("Events URL:", eventsUrl.replace(auth.apikey, "PUBLIC_KEY").replace(auth.hash, "HASH"))
    ...
    console.log("Events API Response Data Structure:", {
      code: data.code,
      status: data.status,
    ...
    if (data.data?.results?.length > 0) {
      console.log("First event sample:", {
        ...
        start: data.data.results[0].start,
        end: data.data.results[0].end,
